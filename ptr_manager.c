...
/*   Updated: 2019/03/12 14:26:15 by ndelhomm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_p(t_struct *t, void *res)
{
	...
	l = ft_strlen(get_addr((unsigned long long)res)) + 2;
	zeros = (t->got_precision) ? t->precision - (l - 2) : 0;
	if (t->width > zeros + 2)
	{
		if (t->width > t->precision && t->precision > (l - 2))
			blanks = t->width - (zeros + l);
		else
			blanks = t->width - l;
